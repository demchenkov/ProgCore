...
namespace Infrastructure.DapperDataAccess.Repositories
{
        ...
        IUserTwoFactorStore<ApplicationUser>, IUserPasswordStore<ApplicationUser>, IUserRoleStore<ApplicationUser>
    {
        ...
        public async Task AddToRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken)
        {
            ...
            await using var connection = new SqlConnection(_connectionString);
            await connection.OpenAsync(cancellationToken);
            await connection.ExecuteAsync("AddUserToRoleByName", new { UserId = user.Id, RoleName = roleName.ToUpperInvariant() },
        ...
        public async Task RemoveFromRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken)
        {
            ...
            await connection.ExecuteAsync(
                $"DELETE FROM [UserRole] WHERE [UserId] = @{nameof(userId)} AND [RoleId] = " +
                        $"(SELECT TOP(1) Id FROM ApplicationRole WHERE NormalizedName = @{nameof(roleName)})",
                new { userId, roleName = roleName.ToUpperInvariant() });
        }
        ...
        public async Task<IList<string>> GetRolesAsync(ApplicationUser user, CancellationToken cancellationToken)
        {
            ...
            var result = await connection.QueryAsync<string>(
                $"SELECT [Name] FROM [UserRole] JOIN [ApplicationRole] ON [RoleId] = [ApplicationRole].[Id] " +
                $"WHERE [UserId] = @{nameof(ApplicationUser.Id)}", user);
        ...
        public async Task<bool> IsInRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken)
        {
            ...
            var result = await connection.QueryFirstOrDefaultAsync<bool>(
                $"SELECT 1 " +
                        $"EXISTS(SELECT * FROM [UserRole] JOIN [ApplicationRole] ON [RoleId] = [ApplicationRole].[Id] " +
                            $"WHERE  [UserId] = @{nameof(ApplicationUser.Id)} AND [ApplicationRole].NormalizedName = @{nameof(roleName)})",
                new { roleName = roleName.ToUpperInvariant(), user.Id });
        ...
        public async Task<IList<ApplicationUser>> GetUsersInRoleAsync(string roleName, CancellationToken cancellationToken)
        {
            ...
            $"SELECT * FROM ApplicationUser " +
                    $"WHERE ApplicationUser.Id IN " +
                        $"(SELECT UserId FROM UserRole WHERE UserRole.RoleId = " +
                            $"(SELECT Id FROM ApplicationRole WHERE ApplicationRole.NormalizedName = @{nameof(roleName)}))",
